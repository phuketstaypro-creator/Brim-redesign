...
- Присланный заказчиком красный фирменный знак подготовлен как прозрачный `public/assets/images/brhk-monogram.png` и используется в header/footer без искажения пропорций. Отдельное видимое название из header удалено по указанию заказчика; доступное имя ссылки остаётся в `aria-label`. Старый `brhk-logo.png` сохранён как технический источник уже выпущенных favicon/manifest icons.
