...
- Перед быстрым доступом и футером серверно рендерится глобальный блок «Полезные ссылки» из `site.usefulLinks`; текстовые ссылки колледжа на ВКонтакте и MAX находятся в футере и берутся из `site.socialLinks`. Сторонние social widgets не используются.
- Присланный заказчиком полный бело-красный логотип побайтово сохранён как `public/assets/images/brhk-logo-full.png` и используется в header/footer без искажения пропорций. Отдельное видимое название из header удалено по указанию заказчика, поскольку оно входит в сам логотип; доступное имя ссылки остаётся в `aria-label`. Старый `brhk-logo.png` сохранён как технический источник уже выпущенных favicon/manifest icons.
...
```text
public/assets/images/brhk-logo-full.png
```
